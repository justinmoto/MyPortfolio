...
const FourthPage = () => {
  ...
  return (
    <div className='h-screen sm:h-[140vh] md:h-[130vh] lg:h-[130vh] xl:h-[110vh] bg-[#34353A] relative tracking-wider' id='achievements'>
        <BgCircle/>
        <div className='relative z-10 h-screen bg-[#d4d4d4] bg-opacity-10 backdrop-filter backdrop-blur-[70px] sm:h-[140vh] md:h-[130vh] lg:h-[130vh] xl:h-[110vh]'>     
            <h1 className='text-[70px] font-extrabold px-[10%] text-white text-center pt-10 sm:text-[50px] md:text-[50px]'>Achievements<span className='text-[#FAD26E]'>.</span></h1>
            {awardinfo.map((award,i) => (
            <div key={i} className='flex justify-center pt-12 sm:pt-7 md:pt-10'>
                <div className='w-[50%] sm:w-[90%] md:w-[90%] lg:w-[80%] xl:w-[80%] 2xl:w-[80%]'>
                    <ul>
                        <li className='text-white relative flex items-baseline gap-10' data-aos={'fade-left'}>
                            <div className='before:absolute before:top-[15%] sm:before:top-[10%] md:before:top-[11%] lg:before:top-[13%]
                            2xl:before:top-[19%] before:left-[30px] before:h-full sm:before:h-[93%] before:w-[6px] before:bg-white'>
                                <h1 className='text-[29px] font-extrabold text-white'>{award.year}</h1>
                            </div>
                            <div className=''>
                                <h1 className='text-[30px] text-[#FAD26E] font-extrabold'>{award.title}</h1>
                                {award.award.map((arc,index) => (
                                <div className='font-bold'>
                                    <p key={index}>{arc}</p>
                                </div>
                                ))}
                                <p className='mt-5 text-[18px]'>{award.info}</p>
                            </div>
                        </li>
                    </ul>
                </div>
